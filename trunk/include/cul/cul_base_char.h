...
static inline char *cul_cnew(size_t size);
static inline char *cul_cresize(char *data, size_t size);
static inline void cul_cfree(char *data);
...
/* Search */
char *cul_clfind(char key, char *data, size_t size, cul_eq_f *eq);
char *cul_clfind_stride(char key, char *data, size_t size, size_t stride, cul_eq_f *eq);
char *cul_cbfind(char key, char *data, size_t size, cul_cmp_f *cmp);
char *cul_cbfind_stride(char key, char *data, size_t size, size_t stride, cul_cmp_f *cmp);
char *cul_clfind_eq(char key, char *data, size_t size);
char *cul_cbfind_eq(char key, char *data, size_t size);
/* Sort */
void cul_csort(char *data, size_t size, cul_cmp_f *cmp);
void cul_csort_stride(char *data, size_t size, size_t stride, cul_cmp_f *cmp);
...
/* Unique */
char *cul_cunique(char *data, size_t size, cul_eq_f *eq);
char *cul_cunique_stride(char *data, size_t size, size_t stride, cul_eq_f *eq);
char *cul_cunique_eq(char *data, size_t size);
char *cul_cunique_eq_stride(char *data, size_t size, size_t stride);

/* Inline definitions */

static inline char *cul_cnew(size_t size) {
	return (char *)cul_malloc(size * sizeof(char));
}

static inline char *cul_cresize(char *data, size_t size) {
	return (char *)cul_realloc(data, size * sizeof(char));
}
