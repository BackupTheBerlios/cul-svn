	...
	ATOM *FUNCTION(find_stride)(const ATOM *data, size_t size, size_t stride, ATOM key) {
	...
	ATOM *FUNCTION(find_stride)(const ATOM *data, size_t size, size_t stride, ATOM key, cul_cmp_f *cmp_f) {
		const ATOM *const last = data + size * stride;
		for( ; data < last; data += stride)
			if( !cmp_f(data, &key) )
				return (ATOM *)data;
	...
	ATOM *FUNCTION(bfind_stride)(const ATOM *data, size_t size, size_t stride, ATOM key) {
		const ATOM *last = data + size * stride;
		for( size >>= 1; size > 0 ; size >>= 1) {
	...
	ATOM *FUNCTION(bfind_stride)(const ATOM *data, size_t size, size_t stride, ATOM key, cul_cmp_f *cmp_f) {
		const ATOM *last = data + size * stride;
		for( size >>= 1; size > 0 ; size >>= 1) {
